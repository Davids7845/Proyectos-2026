...
export class SupabaseWriter implements CalcWriter {
  ...
  async log(entry: CalcLogEntry): Promise<UUID> {
    ...
    const { data, error } = await this.supabase
      ...
      .insert({
        ...
        valor_resultado: String(entry.valor_resultado),
  ...
  async writeCostoProceso(r: ProcesoResult, versionId: UUID, runId: UUID): Promise<void> {
    const { error } = await this.supabase.from("costo_proceso").insert({
      ...
      periodo: r.periodo,
      costo_materia_prima: r.costo_materia_prima != null ? String(r.costo_materia_prima) : null,
      costo_combustible:   r.costo_combustible   != null ? String(r.costo_combustible)   : null,
      costo_energia:       r.costo_energia       != null ? String(r.costo_energia)       : null,
      costo_repuestos:     r.costo_repuestos     != null ? String(r.costo_repuestos)     : null,
      costo_servicios:     r.costo_servicios     != null ? String(r.costo_servicios)     : null,
      costo_total:         String(r.costo_total),
      costo_por_ton:       String(r.costo_por_ton),
      costo_recibido_arrastre:    String(r.costo_recibido_arrastre),
      costo_total_arrastrado:     String(r.costo_total_arrastrado),
      costo_por_ton_arrastrado:   String(r.costo_por_ton_arrastrado),
